...
\begin{wrapfigure}[8]{R}{0.5\textwidth}
	...
	\end{small}
	%\begin{footnotesize}
	%	Example input and output of the image of figure \ref{fig:animal-edge-detection}. $I_{in}$ is the input with a dimension of 3x3 pixels. $I_{out}$ is the output for the center pixel ($x=2, y=2$) of the kernel-processed output image using the kernel filter $K_M$ of equation (\ref{equation:kernal-matrix}).
	%\end{footnotesize}
\end{wrapfigure}
