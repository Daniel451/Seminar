...
\newcommand{\trtitle}{Deep Learning}
...
\frame[t]{
  ...
  \begin{itemize}
  	...
  	\item Deep learning
		\begin{itemize}
			\item utilizing risen computing power
			\item complex tasks
			\item mimic biological neural networks
		\end{itemize}
...
\frame[t]{
  \frametitle{Neural networks: sample figure}
  \begin{figure}
  	\centerline{
...
\frame[t]{
  \frametitle{Kernels}
	\begin{itemize}
		\item Used as filters
		\item Extract information out of an image via a kernel matrix
		\item Collect certain features
	\end{itemize}
}


\frame[t]{
  \frametitle{Kernels: sample figure}
  \begin{figure}
  	\centerline{
...
\frame[t]{
  \frametitle{Subsampling}
	\begin{itemize}
		\item Find representative features, ignores irrelevant information
		\item Shortens the information
		\item Reduces the dependence of features to specific areas
	\end{itemize}
}
...
\frame[t]{
  ...
  \begin{figure}
  	\centerline{
  		\includegraphics[width=1\textwidth]{cnn-face-net.png}
  	}
  	{\tiny\caption{Margarita Osadchy, Matt Miller, Yann LeCun}}
  \end{figure}
}
...
\frame[t]{
  ...
  \begin{itemize}
  	...
  	\item Deactivate randomly chosen neurons with a fixed probability
  	\item Prevent neurons of co-adapting to each other
  	\item Mimic the behavior of biologic neural networks
  \end{itemize}
}


\frame[t]{
  \frametitle{Context stream and fovea stream}
  \begin{figure}
  	\centerline{
  			\includegraphics[width=0.8\textwidth]{google-architecture.png}
  		}
  	{\tiny\caption{Andrej Karpathy et al.}}
  \end{figure}
}
